...
namespace fastJSON
{
    internal sealed class JSONSerializer
    {
        ...
        private void WriteValue(object obj)
        {
            ...
            {
                var dj = obj as DynamicJson;
                if (dj.Dictionary != null && dj.Dictionary.Count > 0) WriteDictionary((IDictionary)dj.Dictionary);
                if (dj.List != null && dj.List.Count > 0) WriteArray(dj.List);
                else
                    WriteArray(new List<object>());
            }
#if !SILVERLIGHT
